...
from molpot.potentials.elec.lib import MeshInterpolator
...
class TestMeshInterpolatorBackward:
    ...
    def test_derivatives(self, interpolation_nodes, random_runs):
        ...
        torch.testing.assert_close(cell.grad, torch.zeros_like(cell.grad), rtol=0, atol=1e-6)
...
def test_wrong_interpolation_nodes_lagrange():
    match = (
        "`interpolation_nodes` is 2 but only values from 3 to 7 for method 'Lagrange' are allowed"
    )
    with pytest.raises(ValueError, match=match):
...
def test_wrong_interpolation_nodes_p3m():
    match = "`interpolation_nodes` is 6 but only values from 1 to 5 for method 'P3M' are allowed"
    with pytest.raises(ValueError, match=match):
