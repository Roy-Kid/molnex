...
## 18b. 2026-04-23 — redesign for paper-faithful QM9 training

Triggered by a QM9 U0 training run that showed **zero descent trend**
under the pre-redesign code. A paper+reference re-audit (see agent report
above) found five blocker-class deviations; this section records which
were fixed, which were kept as documented simplifications, and links each
fix to its test.

**Fixes shipping in `src/molzoo/allegro.py` (single-file rewrite):**

| #  | Original deviation                                          | Fix in rewrite                                                                 | Test                                                                                          |
|----|-------------------------------------------------------------|--------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------|
| 5  | `scalar_mlp` applied SiLU after the final linear             | New `_make_scalar_mlp` helper: ``[Linear→act] × k → Linear``, no trailing act. | `TestPairEmbedding::test_scalar_mlp_has_no_trailing_activation`                               |
| 9  | `u(r_ij)` multiplied post-loop, *after* the residual chain | `u(r_ij)` is now multiplied on `mlp_out` **inside** every `AllegroLayer` before the α-residual add. | `TestAllegroLayer::test_zero_cutoff_freezes_scalar_track` + `TestEnergyInvariants::test_cutoff_smoothness` |
| 13 | Extra post-TP `tensor_env` rescale on the tensor track     | Deleted; tensor track is simply replaced by the TP output (paper behaviour).   | (implicit — no field to maintain)                                                              |
| 14 | Post-layer `scalar × u` hack in `Allegro.forward`           | Removed; the in-layer gate subsumes it.                                        | `TestAllegroEncoder::test_scalar_output_is_rotation_invariant`                                 |
| —  | No 1/√⟨N⟩ normalisation in the edge→atom energy sum         | Moved to new `molpot.heads.EdgeEnergyHead`, which divides each edge by `√avg_num_neighbors`. | `TestNeighborNormalization::test_sqrt_normalization_is_applied`                               |
| —  | No target unit conversion (Hartree ≠ paper's eV)            | New `molix.data.tasks.UnitConvert` task (Hartree → eV).                        | `TestUnitConvert::*`                                                                           |
| —  | No per-atom energy scale — initial MSE dominated by scale   | New `molpot.heads.GlobalRescale`, scale fit from training split std.           | `TestGlobalRescale::test_affine`                                                               |

**Decoupling changes (not algorithmic; for extensibility):**

- The encoder now returns **only** stacked per-layer scalar features
  `(E, L, F_s)` — no energy semantics inside `molzoo.allegro`.
- Energy readout lives in `molpot.heads.EdgeEnergyHead`; per-species /
  global rescaling lives in `molpot.heads.rescale`. Callers wire them
  together explicitly (no registry magic).
- The paper-faithful 2-body MLP shape is generated by a single helper
  `_make_scalar_mlp(in_dim, hiddens, out_dim, activation)` used in both
  `PairEmbedding` and `AllegroLayer`, removing the duplicated loop that
  introduced the original "final SiLU" bug.

**End-to-end correctness tests** (new, `tests/test_molzoo/test_allegro.py`):

- `TestEnergyInvariants::test_translation_invariance` — ``E(x+t) = E(x)``.
- `TestEnergyInvariants::test_rotation_invariance` — ``E(Rx) = E(x)`` for
  five random ``R ∈ SO(3)``.
- `TestEnergyInvariants::test_permutation_invariance` — relabelling
  atoms must not change total energy.
- `TestEnergyInvariants::test_cutoff_smoothness` — removing an atom just
  past ``r_cut`` changes the total energy by < 1 × 10⁻⁴.
- `TestOverfitSingleBatch::test_overfit_constant_target` — single H₂O
  batch must fit to final loss < 1 × 10⁻⁴ under Adam lr=1e-2 in 500 steps.

All six pass on the rewrite; on the pre-redesign code, the
`test_zero_cutoff_freezes_scalar_track` assertion **fails** (proof that
the old code leaked activations past `r_cut` through the residual chain).

**Kept simplifications** (see earlier sections for full rationale):

- Type-embedding concat-vs-product (§4) — kept as concat.
- Single-scalar env weight (§7) — kept.
- `ChannelWiseTensorProduct` with single-channel RHS (§10) — kept.
- Per-layer `(E, L, F)` stack output (§15) — kept; `EdgeEnergyHead` has
  a `layer_reduction` kwarg to choose `"last"` / `"mean"` / `"sum"`.

## 19. Run-linked investigations

_Headings under this section are created by `molzoo-auditor` when a run
