...
| Post-`collate_molecules` `GraphBatch` (what the model forward receives) | **nested `TensorDict`** | `batch["atoms", "Z"]`, `batch["edges", "edge_index"]`, `batch["graphs", "energy"]` |
...
- `types`: TensorDict subclasses — `AtomData`, `EdgeData`, `GraphData`, `GraphBatch` (collated-batch containers only; raw samples are plain dicts)
