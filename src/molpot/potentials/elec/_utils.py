"""Parameter validation utilities for molpot.potentials.elec calculators."""
...
def _validate_parameters(
    ...
    """Validate shapes, dtypes, and devices of calculator input parameters.
    ...
    Args:
        ...
        periodic: Boolean mask for periodic directions ``(3,)``.
        pair_mask: Boolean mask for edge filtering ``(n_edges,)``.
        node_mask: Boolean mask for node filtering ``(n_atoms,)``.
        kvectors: Precomputed k-vectors ``(n_kvecs, 3)``.

    Raises:
        ValueError: If any tensor has wrong shape or device.
        TypeError: If any tensor has wrong dtype.
    """
    ...
    if list(cell.shape) != [3, 3]:
        raise ValueError(
            f"`cell` must be a tensor with shape [3, 3], got tensor with shape {list(cell.shape)}"
        )
    ...
    if cell.device != device:
        raise ValueError(
            f"device of `cell` ({cell.device}) must be same as that of the "
            f"`positions` class ({device})"
        )
    ...
    if charges.dtype != dtype:
        raise TypeError(
            f"type of `charges` ({charges.dtype}) must be same as that of the "
            f"`positions` class ({dtype})"
        )

    if charges.device != device:
        raise ValueError(
            f"device of `charges` ({charges.device}) must be same as that of the "
            f"`positions` class ({device})"
        )
    ...
    if neighbor_indices.shape[1] != 2:
        raise ValueError(
            ...
            f", but got {list(neighbor_indices.shape)} for one structure"
        )
    ...
    if pair_mask is not None:
        ...
        if pair_mask.dtype != torch.bool:
            raise TypeError(f"type of `pair_mask` ({pair_mask.dtype}) must be torch.bool")
    ...
    if node_mask is not None:
        ...
        if node_mask.dtype != torch.bool:
            raise TypeError(f"type of `node_mask` ({node_mask.dtype}) must be torch.bool")
