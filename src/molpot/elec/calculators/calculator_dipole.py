...
from molpot.elec._utils import _validate_parameters
from molpot.elec.lib.kvectors import generate_kvectors_for_ewald
from molpot.elec.potentials import PotentialDipole
...
class CalculatorDipole(torch.nn.Module):
    ...
    def __init__(
        ...
        if not isinstance(potential, PotentialDipole):
            raise TypeError(
                f"Potential must be an instance of PotentialDipole, got "
                f"{type(potential)}"
            )
        ...
        assert (
            ...
            "Either both `lr_wavelength` and `smearing` must be set or both must "
            "be None"
        )
